...
import time
import rospy
from lib_ta_py.new_ekf_imu_2d import imu_ekf_2d
from sensor_msgs.msg import NavSatFix
from sensor_msgs.msg import Imu
from pkg_ta.msg import State_EKF_2D

# Frame
# yawc_compass = np.pi*3/2 + 0.1331 # Android IMU
# yawc_imu = yawc_compass - np.pi # Android IMU
yawc_imu = 0.
# yawc_compass = yawc_imu - np.pi # CEK LAGI MANA YANG BENER !
# yawc_compass = yawc_imu - 0.0 # CEK LAGI MANA YANG BENER !
yawc_compass = - np.pi/2 # CEK LAGI MANA YANG BENER !
# initial_yaw = -3.1380388410843785
gnss_compass_var_max = 0.001
...
pub = rospy.Publisher('/state_2d_new_new', State_EKF_2D, queue_size=1)
...
def callback_gnss(msg_gnss):
    ...
    if not RUN:
        ...
        if np.linalg.norm(ekf.v) >= gnss_compass_vel_min: # m/s
            if msg_gnss.position_covariance[0] <= gnss_compass_var_max:
                if arr_cov[-1] <= gnss_compass_var_max:
                    diff = np.array([gnss_pos[0], gnss_pos[1]]) - arr_pos[-1]
...
def callback_imu(msg_imu):
    ...
    # imu_yaw = euler[-1] # Android IMU
    imu_yaw = euler[-1] #* (-1) # Mounted Razor IMU
    ...
    if not RUN:
        ...
        # ekf.predict(dt, np.array([imu_a.x, imu_a.y]), imu_w.z) # Android IMU
